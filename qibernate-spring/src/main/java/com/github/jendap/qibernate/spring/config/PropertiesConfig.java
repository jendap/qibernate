...
/**
 * Here is configuration of properties used for spring configuration of beans. The properties
 * are read from *.properties files. The property files are read from classpath from subdirectory
 * with the same name as Spring's active profiles.
 *
 */
@Configuration
public class PropertiesConfig {
	...
	@Value("${spring.profiles.active}") String activeEnvironments;
	...
	private List<ClassPathResource> getClassPathResources(final String propertyFileName) {
		...
		if (activeEnvironments != null) {
			for (val location : activeEnvironments.split(",")) {
				resources.add(createClassPathResource(location, propertyFileName));
			}
		}
		return resources;
	}
